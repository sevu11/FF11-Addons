...
addon.name      = 'hush';
addon.author    = 'Hugin, updated by: Sevu11';
addon.version   = '1.5.2';
...
--[[
* Determines if the message is a teleport request.
*
* @param e - The chat event?
* @return {bool} True if teleport request, otherwise false.
--]]
local function is_teleport(e)
    local msg = clean_str(e.message_modified);
    local k = false;

    -- Find teleport requests..
    k = hush.teleports:find_if(function (v, _)
        if (msg:contains(v)) then
            return true;
        end
        return false;
...
--[[
* Determines if the message is related to Eco-Warrior.
*
* @param e - The chat event?
* @return {bool} True if Eco-Warrior message, otherwise false.
--]]
local function is_ecowarrior(e)
    local msg = clean_str(e.message_modified);
    local k = false;

    -- Find Eco-Warrior notifications..
    k = (msg:contains('Eco-Warrior ') and not msg:contains('You completed Eco-Warrior '));
...
--[[
* Determines if the message is related to Highwind.
*
* @param e - The chat event?
* @return {bool} True if Highwind message, otherwise false.
--]]
local function is_highwind(e)
    local msg = clean_str(e.message_modified);
    local k = false;

    -- Find Highwind notifications..
    k = (msg:contains('Highwind ') and not msg:contains('You received '));
...
--[[
* Determines if the message contains any of the specified keywords.
*
* @param e - The chat event?
* @return {bool} True if the message contains any keywords, otherwise false.
--]]
local function contains_keywords(e)
    local msg = clean_str(e.message_modified);
    for _, keyword in pairs(hush.settings.keywords) do
        if msg:lower():contains(keyword:lower()) then
            return true;
        end
    end
    return false;
...
ashita.events.register('text_in', 'text_in_cb', function (e)
    ...
    -- Hush messages with keywords..
    if (hush.settings.keywords and #hush.settings.keywords > 0) then
        if (contains_keywords(e)) then
            e.blocked = true;
            return;
        end
    end
...
ashita.events.register('d3d_present', 'd3d_present_cb', function ()
    ...
    if (imgui.Begin('Hush Settings##HushSettingsWindow', hush.guiOpen, ImGuiWindowFlags_AlwaysAutoResize)) then
        ...
        if imgui.Checkbox('Horizon Mode ##Hush_horizon_mode_Check', { hush.settings.horizon_mode }) then
            hush.settings.horizon_mode = not hush.settings.horizon_mode;
        end

        if hush.settings.horizon_mode then
        ...
        if imgui.Checkbox('Hush Selected keywords ##HushKeywordsCheck', { hush.settings.keywords_enable }) then
            hush.settings.keywords_enable = not hush.settings.keywords_enable;
            update_settings();
        end

        -- Display Keywords
        if hush.settings.keywords_enable then
            imgui.Text("\nTo add more keywords, see your settings file:\n\nsettings[\"keywords\"][#] = \"<keyword>\";\n\n")
            imgui.Text("Active keywords:");

            -- List current keywords
            if #hush.settings.keywords > 0 then
                for i, keyword in ipairs(hush.settings.keywords) do
                    imgui.Text(string.format("%d: %s", i, keyword));
                end
            else
                imgui.Text("No keywords set.");
            end
        end
        if imgui.Button('Save Settings') then
            update_settings()
            hush.guiOpen[1] = false
        end
...
--[[
* event: command
* desc : Event called when the user enters a command.
--]]
